//! `lfm_vl_vision` — interactive multi-question REPL using the
//! **Liquid AI LFM2.5-VL 1.6B** vision-language model. Smaller than
//! Gemma 4 (~1 GB vs ~5 GB), so it runs comfortably on CPU and on
//! older Apple Silicon.
//!
...
fn main() -> Result<()> {
    ...
    if let Some(p) = args.next() {
        // 5th positional = first prompt, no REPL
        return run_single_turn(&hf_repo, &text_filename, &mmproj_filename, &image, &p);
    }
    ...
    loop {
        ...
        if let Some(rest) = input.strip_prefix("/image ") {
            ...
            image = ensure_test_image(&new_path)?;
...
/// One-shot helper used when the user passes 5 positional arguments.
fn run_single_turn(
    hf_repo: &str,
    text_filename: &str,
    mmproj_filename: &str,
